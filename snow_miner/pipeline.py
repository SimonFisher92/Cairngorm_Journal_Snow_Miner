...
def process_pdf(pdf_path: str, out_dir: str = "out", include_date_col: bool = True) -> Optional[str]:
    ...
    # Made a change here- One call over the whole document (the analyzer will chunk internally as needed).
    ...
    with open(out_path, "w", newline="", encoding="utf-8") as f:
        ...
        for r in rows:
            ...
            if include_date_col:
                # gpt_analyse returns "date" (string or None)
                row["date"] = r.get("date")
            writer.writerow(row)
